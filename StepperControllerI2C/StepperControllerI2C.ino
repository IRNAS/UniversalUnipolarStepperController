...
Universal Unipolar Sttepper Controller I2C version for use with 28BYJ48 stepper motors
...
//note distribution of sizes is important for packing
typedef __attribute__ ((packed)) struct struct_motor_t{
  ...
  int32_t current_y;
  int32_t current_f;
  uint8_t status_x; // 24 offset
  uint8_t status_y;
  uint8_t status_f;
  uint8_t empty3;
  uint8_t flash_status;
  uint8_t flash_write_count; 
  uint8_t command; // 30 offset
  uint8_t gpio;
...
void home_axis(AccelStepper *stepper,long *next,boolean *home_enable, boolean dir, uint8_t status_axis){
  ...
  if(status_axis==0x10 | stepper->currentPosition()==-100000){
    stepper->move(0);
...
void receiveEvent(int aCount){
  char data_length=0;
  //first determine the offset through command applies to write and read
  if(aCount > 0){
    offset=Wire.read();
    aCount--; //decrease number of available bytes
  }
  //first determine the offset through command applies to write and read
  if(aCount > 0){
  ...
  for(char j=0;j<aCount;j++){
    motor_rx.i[offset+j]=Wire.read();
  }
    
  //below is not optimal, further optimization needed
  if(offset==0&data_length>=12){
  ...
  if(offset==30){
    ...
    motor.u.gpio=motor_rx.u.gpio;
    digitalWrite(GPIO,gpio_state);
  }
}


void requestEvent(){
  if(offset>=sizeof(struct_motor_t)){
    Wire.write(0); //reply null
    //Serial.print(" 0x");
    //Serial.print(1,DEC);
  }
  else{
    Wire.write(motor.c[offset++]); //reply byte and increment offset
    //Serial.print(" 0x");
    //Serial.print(motor_rx.i[offset-1],DEC);
  }
   //Serial.println("");
  //dump_i2c=HIGH;
}
...
void loop(){
  ...
  /*if(dump_i2c){
    dump_i2c=LOW;
    Serial.print(motor.u.current_x);
    Serial.print(" ");
    Serial.print(motor.u.current_y);
    Serial.print(" ");
    Serial.println(motor.u.current_f);
  }*/


  if(timeout_report<millis()){
    //dump_motor_variables();
    timeout_report=millis()+500;
  }
  ...
  */
  ...
  // automatic store position after idle for a while
...
void process_command(){
    ...
    3 - home y
    4 - home f
    5 - configure current position
    6 - configure speed and acceleration
    */
    ...
    case 4: // home f
      motor_rx.u.command=0;
      break;
    case 5: // configure current position
      stepperX.setCurrentPosition(motor_rx.u.next_x);
      stepperY.setCurrentPosition(motor_rx.u.next_y);
      stepperF.setCurrentPosition(motor_rx.u.next_f);
//        //store to flash
//      flash_erase();
//      flash_write();
      motor_rx.u.command=0;
      break;
    case 6: // configure speed and accel
...
void dump_motor_variables(){
  ...
  Serial.println(motor.u.current_x);
  Serial.print("Next x: ");
  Serial.println(motor.u.next_x);
  Serial.print("Status x: 0x");
  Serial.println(motor.u.status_x,HEX);
  Serial.print("Current y: ");
  Serial.println(motor.u.current_y);
  Serial.print("Next y: ");
  Serial.println(motor.u.next_y);
  Serial.print("Status y: 0x");
  Serial.println(motor.u.status_y,HEX);
  Serial.print("Current f: ");
  Serial.println(motor.u.current_f);
  Serial.print("Next f: ");
  Serial.println(motor.u.next_f);
  Serial.print("Status f: ");
  Serial.println(motor.u.status_f);
  Serial.print("Command: ");
  Serial.println(motor.u.command);
  Serial.print("Gpio: ");
  Serial.println(motor.u.gpio);
  Serial.print("Flash write: ");
  Serial.println(motor.u.flash_write_count);
  Serial.print("Accel: ");
  Serial.println(motor.u.motor_accel);
  Serial.print("Speed: ");
  Serial.println(motor.u.motor_speed);
  Serial.print("Home x: ");
  Serial.println(home_enable_x);
  Serial.print("Home y: ");
  Serial.println(home_enable_y);
  
  /*for(char j=0;j<sizeof(struct_motor_t);j++){
    Serial.print(" ");
    Serial.print(motor.c[j],DEC);
  }*/
}
